...
The Portable Portal is not granted automatically.
...
- purchase creates/owns one physical Portable Portal unit subject to the normal inventory/no-duplication rules,
- once purchased, that unit may be equipped in any unlocked world and consumes one turret slot.
...
## 37. Companion incapacitation semantics

### GZ-COMP-010 — Nóma never reaches permanent DEAD state
**APPROVED**

When Nóma reaches terminal health during a sortie:
- Nóma enters DOWNED/DISABLED,
- Nóma does not transition to permanent DEAD,
- Nóma never requires character revival payment,
- same-sortie rescue/recovery rules continue to apply,
- intact Nóma equipment follows normal recovery rules,
- destroyed equipment remains LOST under normal destruction rules.

This supersedes any generic companion wording that implied Nóma could become permanently DEAD.

### GZ-COMP-011 — Ally DOWNED becomes DEAD only from additional damage
**APPROVED**

The ally does not die from an automatic bleedout timer.

While DOWNED:
- the ally remains rescueable indefinitely within the sortie unless the state is otherwise resolved,
- additional enemy damage against the DOWNED ally can transition the ally to DEAD,
- ordinary passage of time alone does not transition DOWNED -> DEAD.

A DEAD ally still follows the paid-revival contract before redeployment.
