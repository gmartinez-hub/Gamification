...
## Measurement Pass 1 — real asset evidence

Generated by `scripts/runtime-v2-measure-assets.mjs` through the repository measurement workflow.

Evidence report:
- `docs/runtime-v2/evidence/asset-measurements-v1.json`
- `docs/runtime-v2/evidence/asset-measurements-v1.md`

### Confirmed human anchor
Astronaut raw height = **1.89899 m**. V2 canonical anchor = **1.90 m**.

### Player ship legacy semantic envelope
Evidence from raw module bounds + the old measured staging transforms:
- Front-only: **4.491 m** long
- Front + Final: **8.744 m**
- Front + Middle + Final: **12.869 m**
- Front + 5 Middle + Final: **29.369 m**

These are evidence values, not automatic V2 constants.

### Alien ship relation
Legacy declared runtime length = **25.7335 m**, almost exactly **2×** the measured standard Front+Middle+Final player ship envelope.

This strongly supports the approved semantic relation `alien ship > player ship` and provides a useful initial target relation for ScaleProfile review.

### Hangar relation
Legacy service-bay staging used **18×** the normalized source asset:
- approximately **34.188 × 15.494 × 32.235 m**

This is coherent with:
- standard ~12.869 m player ship having large clearance,
- extreme ~29.369 m long composition nearly reaching/extending beyond the core service envelope.

Treat 18× as measured reference. V2 may reproduce the semantic dimensions cleanly without copying the old staging code.

### Cockpit diagnosis
The uncut source cabin shell leaves approximately **1.901 m** of rear depth beyond the standing astronaut envelope.

The current legacy rear clipping cuts approximately **0.619 m** into the standing astronaut envelope.

Initial classification: **SOURCE_EXISTS_RUNTIME_HIDES**.

Therefore cockpit-depth remediation begins as a Three/runtime composition ticket, not a Blender replacement-shell ticket. Blender work remains conditional on visual proof finding genuinely missing enclosure geometry.

### Portal / ring
No ring GLB exists in main.

Current procedural legacy reference:
- event-horizon diameter: **6.8 m**
- visual disc outer diameter: **15.6 m**

This is evidence only. Ring aperture must be validated against V2 semantic ship cross-section and the chosen transit formation.

### Missing
No mothership GLB exists in main. Exact mothership dimensions remain PRODUCE/MEASURE after asset creation.
