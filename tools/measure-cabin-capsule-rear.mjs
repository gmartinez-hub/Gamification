// Run from repository root. Missing rear intersections are recorded as null.
...
for(const z of [-1.9,-1.8,-1.7,-1.6,-1.5,-1.4,-1.3]){const radii=[];for(let j=0;j<24;j++){const a=j*Math.PI/12;ray.set(new T.Vector3(0,0,z),new T.Vector3(Math.cos(a),Math.sin(a),0));const hits=ray.intersectObject(mesh);const h=hits.at(-1);radii.push(h?+h.distance.toFixed(3):null);}rows.push({z,radii});console.log(z,Math.min(...radii.filter(x=>x)),radii.filter((_,i)=>i%6===0));}
fs.writeFileSync('.superpowers/sdd/2026-09-17-final-expedition/cabin-evidence/capsule-rear-envelope.json',JSON.stringify(rows,null,2));
