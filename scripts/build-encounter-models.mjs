...
if(inputs.length!==4) throw new Error('Usage: node scripts/build-encounter-models.mjs BIKE_GLB RIDER_CLIPS_GLB ALIEN_GLB ALIEN_SHIP_GLB');
const dest=new URL('../assets/runtime/encounter-models/',import.meta.url);
await mkdir(new URL('textures/',dest),{recursive:true});
const hash=b=>createHash('sha256').update(b).digest('hex'),manifest=[];
for(const [file,source] of [['bike.glb',inputs[0]],['bike-rider.glb',inputs[1]],['alien.glb',inputs[2]],['alien-ship.glb',inputs[3]]]){
