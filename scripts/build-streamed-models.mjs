...
const base=new URL('../assets/runtime/',import.meta.url),dest=new URL('streamed-models/',base);
await mkdir(new URL('textures/',dest),{recursive:true});
const hash=b=>createHash('sha256').update(b).digest('hex'),manifest=[];
for(const file of (await readdir(new URL('models/',base))).filter(f=>f.endsWith('.glb'))){
 const bytes=await readFile(new URL('models/'+file,base)),n=bytes.readUInt32LE(12),g=JSON.parse(bytes.subarray(20,20+n)),bin=bytes.subarray(28+n);
