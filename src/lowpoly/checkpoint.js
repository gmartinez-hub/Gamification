...
export const DEFAULT_SETTINGS = Object.freeze({ firstPerson: true, soundEnabled: false, effectsVolume: .75, ambienceVolume: .4, introSeen: false });
  ...
  return {
    load({ seed } = {}) {
      try {
        ...
        if (!validCheckpoint(record) || record.version !== 1 || (seed !== undefined && seed !== record.seed)) return null;
        return { version: 1, seed: record.seed, sector: record.sector, complete: record.complete,
          gemRecovered: record.gemRecovered, settings: settingsFrom(record.settings) };
      } catch { return null; }
    },
    save(state, settings) {
      try {
        const record = { version: 1, seed: state.seed, sector: state.sector,
