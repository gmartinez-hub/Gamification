...
const turboAbility = document.querySelector("#turboAbility");
const turboAbilityLabel = document.querySelector("#turboAbilityLabel");
const turboAbilityState = document.querySelector("#turboAbilityState");
const turboAbilityMeter = document.querySelector("#turboAbilityMeter");
const stabilizerAbility = document.querySelector("#stabilizerAbility");
const stabilizerAbilityState = document.querySelector("#stabilizerAbilityState");
const stabilizerAbilityMeter = document.querySelector("#stabilizerAbilityMeter");
...
const objectiveGuide = document.querySelector("#objectiveGuide");
const objectiveGuideArrow = document.querySelector("#objectiveGuideArrow");
const objectiveGuideLabel = document.querySelector("#objectiveGuideLabel");
const objectiveGuideDistance = document.querySelector("#objectiveGuideDistance");
...
const storyBeatKicker = document.querySelector("#storyBeatKicker");
const storyBeatTitle = document.querySelector("#storyBeatTitle");
const storyBeatBody = document.querySelector("#storyBeatBody");
...
const CAMERA_CUES = {
  ...
  mission: { zoom: 1.012, attack: 0.42, hold: 0.8, release: 0.55 },
  relic: { zoom: 1.024, attack: 0.34, hold: 0.9, release: 0.62 },
  gem: { zoom: 1.032, attack: 0.28, hold: 0.7, release: 0.70 },
  pickup: { zoom: 1.018, attack: 0.30, hold: 0.65, release: 0.62 },
  gate: { zoom: 0.985, attack: 0.55, hold: 0.8, release: 0.70 },
  corridor: { zoom: 0.98, attack: 0.65, hold: 3.0, release: 0.55 },
};
...
function moveWorldOffset(x, y) {
  ...
  // Landmarks are physical encounter spaces. Keep the camera subject outside
  // their footprint so the ship cannot visually pass through the structure.
  for (const landmark of authoredScenarioLandmarks || []) {
    if (landmark.userData.profileStage !== state.worldStageIndex || landmark.userData.finalPortal) continue;
    const base = landmark.userData.base;
    const away = new THREE.Vector2(state.worldOffset.x - base.x, state.worldOffset.y - base.y);
    const minimumDistance = landmark.userData.finalNode ? 0.72 : 2.45;
    if (away.length() >= minimumDistance) continue;
    if (away.lengthSq() < 0.0001) away.set(-x || 1, -y || 0);
    away.setLength(minimumDistance);
    state.worldOffset.set(base.x + away.x, base.y + away.y);
  }
  syncWorldProgress();
}
...
const missionAudioItems = {
  ...
  ambient_oceanic: {
    path: "assets/runtime/final-showable/audio/ambient_oceanic_45s.wav",
    volume: 0.052,
    loop: true,
  },
  ambient_mechanical: {
    path: "assets/runtime/final-showable/audio/ambient_mechanical_45s.wav",
    volume: 0.050,
    loop: true,
  },
  ambient_dark_crater: {
    path: "assets/runtime/final-showable/audio/ambient_dark_crater_45s.wav",
    volume: 0.046,
    loop: true,
  },
  ambient_relic_core: {
    path: "assets/runtime/final-showable/audio/ambient_relic_core_45s.wav",
    volume: 0.052,
    loop: true,
  },
  route_detected_ping: {
...
function updateBiomeAmbientAudio(delta) {
  if (!missionAudio.ready || !mission01.started) return;
  const activeIndex = THREE.MathUtils.clamp(currentWorldProfileIndex(), 0, BIOME_AMBIENT_IDS.length - 1);
  BIOME_AMBIENT_IDS.forEach((id, index) => {
    if (index === activeIndex && !missionAudio.loops[id]) {
      const loop = playMissionAudio(id);
      if (loop) loop.volume = 0;
    }
...
function updateMissionHud(status, progress = "", subtitle = "") {
  ...
  missionHudInstruction = progress;
...
function renderMissionProgress(progress = "") {
  ...
  if (mission01.started) {
    ...
    if (missionHudInstruction) {
      const instruction = document.createElement("div");
      instruction.className = "mission-instruction";
      instruction.textContent = missionHudInstruction;
      missionProgress.append(instruction);
    }
    missionProgress.append(
...
function resetMissionTarget(target, active = false) {
  ...
  target.userData.spawnRevealTime = active ? 0 : -1;
...
function resetStageMission(stageIndex) {
  ...
  gemPickupState.active = false;
  relicGroup.scale.setScalar(1);
  relicGroup.rotation.z = 0;
...
function activateStageTargets(stageIndex) {
  ...
  const landmarkPending = missionPrimaryLandmarkPending(stageIndex);
  ...
  for (const [index, target] of pool.small.entries()) {
    ...
    target.userData.discovery = {
      ...
      position: { x: 0, y: 0 },
      state: !landmarkPending && index === 0 ? "signal_detected" : "unknown",
      signalStrength: 0,
...
function startMissionForStage(stageIndex) {
  ...
  enterShipMode();
  const primary = primaryLandmarkEntryForStage(safeStageIndex);
  const accent = scenarioForStage(safeStageIndex).accent;
  const primaryEncounter = primary ? encounterForLandmark(primary.landmark.id) : null;
  showStoryBeat(
    `ACTO ${safeStageIndex + 1} · RASTREO`,
    primary && !primary.activated ? primary.landmark.name : config.name,
...
const authoredHeroTextures = {
  ...
  "assets/runtime/gravedad-zero/planets/planet_aurora_gas_albedo.png": worldTextures.auroraGas,
  "assets/runtime/three-textures/ocean-world-bright-color.png": worldTextures.oceanWorld,
  "assets/runtime/three-textures/ocean-color.png": worldTextures.oceanColor,
  "assets/runtime/three-textures/network-planet-dark-color.png": worldTextures.networkPlanet,
  "assets/runtime/three-textures/asteroid-crater-magenta-color.png": worldTextures.craterWorld,
  "assets/runtime/three-textures/dark-crater-color.png": worldTextures.darkCrater,
};
...
const scenarioDiscoveryTargets = new Map(
  SCENARIOS.flatMap((scenario) => scenario.landmarks.map((landmark) => {
    ...
    return [landmark.id, {
      ...
      activated: restoredState === "targetable",
      landmark,
      object,
    }];
  })),
...
function missionPrimaryLandmarkPending(stageIndex = mission01.currentStageIndex) {
  if (params.get("qaSkipLandmark") === "1") return false;
  if (!mission01.started || mission01.state !== "small_asteroids") return false;
  const entry = primaryLandmarkEntryForStage(stageIndex);
  return Boolean(entry && !entry.activated);
}
...
const LANDMARK_ENCOUNTERS = {
  fractured_beacon: { range: 0.94, requiresStabilizer: false, reward: "RUTA DE OBJETIVOS REVELADA" },
  orbital_ruins: { range: 0.88, requiresStabilizer: false, reward: "ESCUDO +24" },
  broken_ring: { range: 0.98, requiresStabilizer: false, reward: "CORREDOR MECÁNICO TRAZADO" },
  scanner_array: { range: 0.92, requiresStabilizer: true, reward: "OBJETIVOS MÓVILES MARCADOS" },
  synthetic_rift: { range: 0.98, requiresStabilizer: true, reward: "VENTANA GRAVITACIONAL SEGURA" },
  gravity_tower: { range: 0.92, requiresStabilizer: true, reward: "ESTABILIZADOR RECARGADO" },
};

function encounterForLandmark(id) {
  return LANDMARK_ENCOUNTERS[id] || { range: 0.58, requiresStabilizer: false, reward: "SEÑAL SINCRONIZADA" };
}
...
function activateScenarioLandmark(entry) {
  ...
  for (const target of activeTargets) {
    const revealsTargets = ["fractured_beacon", "broken_ring", "scanner_array", "synthetic_rift"].includes(entry.landmark.id);
    target.userData.discovery.state = revealsTargets ? "targetable" : target.userData.discovery.state === "unknown" ? "signal_detected" : target.userData.discovery.state;
    target.userData.discovery.signalStrength = Math.max(target.userData.discovery.signalStrength || 0, revealsTargets ? 1 : 0.58);
    target.userData.discovery.scanProgress = Math.max(target.userData.discovery.scanProgress || 0, revealsTargets ? 1 : 0.18);
    if (revealsTargets) {
  ...
  if (entry.landmark.role === "secondary") {
    v2Runtime.shield.value = Math.min(100, v2Runtime.shield.value + (entry.landmark.id === "orbital_ruins" ? 24 : 16));
    shipShieldFx.trigger(0.72);
  }
  if (["gravity_tower", "synthetic_rift"].includes(entry.landmark.id)) scenarioGravity.rechargeStabilizer();
  entry.object.userData.encounterComplete = true;
  ...
  updateRobotPanel();
  const primaryMissionLandmark =
    entry.landmark.role === "primary" &&
    mission01.started &&
    mission01.state === "small_asteroids" &&
    entry.stageIndex === mission01.currentStageIndex;
  updateMissionHud(
    entry.landmark.name,
    primaryMissionLandmark ? "SEÑALES DE FRAGMENTOS REVELADAS" : encounterForLandmark(entry.landmark.id).reward,
    copy,
  );
  showStoryBeat(
    primaryMissionLandmark ? `ACTO ${entry.stageIndex + 1} · SEÑAL ABIERTA` : "LANDMARK SINCRONIZADO",
    entry.landmark.name,
    primaryMissionLandmark
      ? "La estructura trazó la ruta. Seguí las señales cyan y fijá cada fragmento."
      : encounterForLandmark(entry.landmark.id).reward,
    { duration: 3800, accent: scenarioForStage(entry.stageIndex).accent },
  );
  saveProgress();
}
...
function updateScenarioDiscovery(rawDelta) {
...

  {
    ...
    const candidates = [...scenarioDiscoveryTargets]
      ...
      const visualPosition = entry.object
        ...
        return {
          ...
          entry,
          visualPosition,
          distance: visualPosition.distanceTo(player),
    ...
    for (const { id, entry, visualPosition, distance: screenDistance, encounter } of candidates) {
      const previousState = entry.state;
      const dx = visualPosition.x - player.x;
      const dy = visualPosition.y - player.y;
      const distance = Math.max(0.001, Math.hypot(dx, dy));
      const isSelected = selected?.id === id;
      const inRange = screenDistance <= encounter.range;
      const stabilizerReady = !encounter.requiresStabilizer || scenarioGravity.status.active;
      const scansThisTarget = scanHeld && isSelected && inRange && stabilizerReady;
      if (scanHeld && isSelected && !inRange) scanBlockedReason = `ACERCATE · ${Math.ceil(screenDistance * 10)}u`;
      if (scanHeld && isSelected && inRange && !stabilizerReady) scanBlockedReason = "PRESIONÁ ESPACIO · ESTABILIZADOR REQUERIDO";
      ...
      if (object) {
        ...
        object.traverse((child) => {
          ...
          const role = child.userData.landmarkRole;
          child.material.emissiveIntensity = role === "core" || role === "signal" || role === "encounter-node"
            ? 0.14 + entry.scanProgress * 0.34
            : 0.07 + entry.scanProgress * 0.08;
        });
      }
  ...
  if (scanHeld && activeTarget && scanBlockedReason) {
    ...
    robotCompanion.message = `${activeTarget.landmark.name} · ${scanBlockedReason}`;
    qaTelemetry.lastCompanionMessage = robotCompanion.message;
  } else if (scanHeld && activeTarget && strongestSignal > 0.12 && activeTarget.state !== "targetable") {
    robotCompanion.focus = "discovery";
    robotCompanion.focusTimer = Math.max(robotCompanion.focusTimer, 0.25);
    robotCompanion.message = `${activeTarget.landmark.name} · NODOS ${Math.min(3, Math.floor(strongestProgress * 3) + 1)}/3 · ${Math.round(strongestProgress * 100)}%`;
    qaTelemetry.lastCompanionMessage = robotCompanion.message;
  }
}
...
astronautGroup.add(astronautThrusters);
const evaMicroJetGroup = new THREE.Group();
for (const side of [-1, 1]) {
  const geometry = new THREE.BufferGeometry();
  geometry.setAttribute("position", new THREE.Float32BufferAttribute([
    -0.022, side * 0.020, 0.020,
    -0.112, side * 0.034, 0.020,
    -0.112, side * 0.010, 0.020,
  ], 3));
  ...
  const jet = new THREE.Mesh(geometry, new THREE.ShaderMaterial({
    uniforms: {
      uColor: { value: new THREE.Color(side > 0 ? 0x8df4ff : 0x7f9cff) },
      uOpacity: { value: 0.34 },
    },
    vertexShader: `
      varying vec2 vUv;
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float hot = 1.0 - smoothstep(0.0, 0.42, vUv.x);
        gl_FragColor = vec4(mix(uColor, vec3(0.92, 0.99, 1.0), hot), uOpacity * fade);
      }
    `,
    transparent: true,
    blending: THREE.AdditiveBlending,
  ...
  jet.renderOrder = 19;
  evaMicroJetGroup.add(jet);
}
evaMicroJetGroup.visible = false;
astronautGroup.add(evaMicroJetGroup);
const stabilizerFieldFx = new StabilizerFieldFx();
scene.add(stabilizerFieldFx);
...
const NAVIGATION_PATH_POINT_COUNT = 6;
...
const navigationPathState = { targetKind: "none", targetId: null, distance: 0, worldDistance: null };
...
if (params.has("qaCdp") || params.get("debugAim") === "1") {
  window.__gzDebug = () => ({
    ...
    objectiveGuide: {
      visible: !!objectiveGuide && !objectiveGuide.hidden,
      label: objectiveGuideLabel?.textContent || "",
      distance: objectiveGuideDistance?.textContent || "",
    },
    storyBeat: {
      visible: !!storyBeat && !storyBeat.hidden && storyBeat.classList.contains("is-visible"),
      kicker: storyBeatKicker?.textContent || "",
      title: storyBeatTitle?.textContent || "",
      body: storyBeatBody?.textContent || "",
    },
...
const interstageOverlay = new THREE.Mesh(
  new THREE.PlaneGeometry(2, 2),
  new THREE.ShaderMaterial({
    uniforms: {
      uOpacity: { value: 0 },
      uColor: { value: new THREE.Color(0x78eaff) },
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float vignette = smoothstep(0.30, 1.08, d);
        float tunnel = smoothstep(0.98, 0.42, d) * smoothstep(0.08, 0.40, d);
        float alpha = uOpacity * (vignette * 0.72 + tunnel * 0.12);
        gl_FragColor = vec4(mix(vec3(0.005, 0.008, 0.02), uColor, tunnel * 0.26), alpha);
      }
    `,
    transparent: true,
    depthTest: false,
...
interstageOverlay.scale.set(viewport.aspect, 1, 1);
interstageOverlay.position.z = 0.16;
interstageOverlay.renderOrder = 1000;
interstageGroup.add(interstageOverlay);

const interstageRandom = seedRandom(9041);
const interstageStreaks = Array.from({ length: 14 }, (_, index) => {
  ...
  const streak = new THREE.Mesh(
    new THREE.PlaneGeometry(0.32 + interstageRandom() * 0.30, 0.004 + interstageRandom() * 0.005),
    material,
  );
  streak.userData.angle = (index / 14) * Math.PI * 2 + (interstageRandom() - 0.5) * 0.24;
  streak.userData.phase = interstageRandom();
  streak.userData.speed = 0.58 + interstageRandom() * 0.42;
...
const gemPickupState = {
  active: false,
  time: 0,
  duration: 0.92,
  start: new THREE.Vector2(),
  hand: new THREE.Vector2(),
  ship: new THREE.Vector2(),
};
...
robotFaceEmissiveMaterial.opacity = 0.012;
...
function robotTutorialContent(missionState = mission01.state) {
  if (robotCompanion.focus === "turbo" && robotCompanion.focusTimer > 0) {
    const turbo = currentTurboUnlock();
    const astronautMode = state.controlMode === "astronaut";
    return {
      label: astronautMode ? "MICROPROPULSOR EVA" : "TURBO DISPONIBLE",
      goal: astronautMode ? "El astronauta también dispone de impulso controlado." : `${turbo.label} · Gemas ${mission01.gems}/3.`,
      action: astronautMode ? "Mantené F mientras usás WASD. El tether limita el alcance." : "Mantené F para acelerar.",
      tip: astronautMode ? "La estela corta confirma el impulso sin confundirse con un disparo." : "Cada gema mejora el impulso sin perder control de rumbo.",
    };
  }

  if (robotCompanion.focus === "stabilizer" && robotCompanion.focusTimer > 0) {
    const status = scenarioGravity.status;
    return {
      label: "ESTABILIZADOR GRAVITACIONAL",
      goal: status.active ? "Gravedad reducida al 24%." : status.cooldown > 0 ? `Recarga ${Math.ceil(status.cooldown)}s.` : "Sistema listo.",
      action: "Presioná ESPACIO dentro de un campo para conservar trayectoria.",
      tip: "Scanner Array, Synthetic Rift y Gravity Tower requieren estabilización activa.",
    };
  }
  ...
  if (missionState === "small_asteroids") {
    ...
    const primary = primaryLandmarkEntryForStage(mission01.currentStageIndex);
    const landmarkPending = missionPrimaryLandmarkPending();
    const stageAction = landmarkPending
      ? `Piloteá hasta ${primary?.landmark.name || "el landmark"} y mantené E para escanear sus 3 nodos.`
      : mission01.currentStageIndex === 0
      ? "Seguí cada señal cyan; el astronauta se despliega al clickear un fragmento."
    ...
    return {
      ...
      goal: landmarkPending
        ? `Sincronizá ${primary?.landmark.name || "el landmark"} para revelar la ruta.`
        : `Localizá ${mission01.smallRequired} fragmentos de señal y neutralizá el núcleo principal.`,
      action: stageAction,
      tip: landmarkPending
        ? "Los tres nodos se encienden durante el escaneo; al completarlo el sector cambia."
        : mission01.currentStageIndex === 2
...
function updateCompanionDirective() {
  ...
  if (mission01.state === "collecting_relic") {
    companionDirectiveTitle.textContent = "GEMA MAGNETIZADA";
    companionDirectiveAction.textContent = "Transferencia mano → nave en curso";
    return;
  }
  if (mission01.state === "small_asteroids") {
    const primary = primaryLandmarkEntryForStage(mission01.currentStageIndex);
    if (missionPrimaryLandmarkPending() && primary) {
      companionDirectiveTitle.textContent = primary.landmark.name;
      companionDirectiveAction.textContent = "Piloteá hasta la señal · mantené E para escanear";
    } else {
      companionDirectiveTitle.textContent = `FRAGMENTOS ${mission01.smallDestroyed}/${mission01.smallRequired}`;
      companionDirectiveAction.textContent = "Seguí la ruta cyan · clic despliega EVA y dispara";
    }
    return;
  }
...
function syncRobotCompanion(missionState = mission01.state) {
  ...
  if (missionState === "small_asteroids") {
    const primary = primaryLandmarkEntryForStage(mission01.currentStageIndex);
    if (missionPrimaryLandmarkPending() && primary) {
      setRobotCompanionState("hint", `SEÑAL PRIMARIA · ${primary.landmark.name}`);
      return;
    }
...
function showInvalidAim(point, message = "OBJETIVO FUERA DE RANGO") {
  ...
  setCompanionAimFeedback(message);
}
...
function beginAimAssistTarget(target, clickPoint) {
  ...
  const origin = shooter === "astronaut"
    ? state.controlMode === "astronaut" ? astronautState.position : currentAstronautAnchor()
    : state.position;
  ...
  if (shooter === "astronaut" && astronautSprite) enterAstronautMode("combat");
...
function clearAimAssistSprites() {
  ...
  if (recallAstronaut) {
    enterShipMode();
    setCompanionAimFeedback("EVA RECUPERADO · PILOTAJE RESTABLECIDO");
  }
}
...
function updateAimAssist(delta, elapsed) {
  ...
  aimVignette.material.opacity = (aimAssist.mode === "major" ? 0.055 : 0.035) * Math.sin(Math.PI * Math.min(1, progress));
  aimField.position.set(origin.x, origin.y, 0.16);
  const localizedFieldScale = aimAssist.mode === "major" ? 0.68 : 0.48;
  aimField.scale.setScalar(localizedFieldScale * (0.94 + orientation * 0.06));
  aimField.material.opacity = (aimAssist.mode === "major" ? 0.12 : 0.08) * Math.sin(Math.PI * Math.min(1, progress));
...
function resizeShip() {
  ...
  shipAura.scale.set(width * 1.38, height * 1.10, 1);
}
...
function updateAbilityHud() {
  ...
  const visible = mission01.started && !mission01.finalComplete && !state.transition && !(gameMenu && !gameMenu.hidden);
  abilityHud.hidden = !visible;
  if (!visible) return;
  ...
  turboAbilityMeter?.style.setProperty("--ability-level", speedState.turboActive ? "100%" : "0%");

  const stabilizer = scenarioGravity.status;
  const activeLevel = stabilizer.active ? THREE.MathUtils.clamp(stabilizer.remaining / 2.5, 0, 1) : 0;
  const cooldownLevel = stabilizer.cooldown > 0 ? THREE.MathUtils.clamp(1 - stabilizer.cooldown / 8, 0, 1) : 1;
  ...
  stabilizerAbilityMeter?.style.setProperty(
    "--ability-level",
    `${Math.round((stabilizer.active ? activeLevel : cooldownLevel) * 100)}%`,
  );
}
...
function updateGateGuide() {
  ...
  // Narrative beats own the top-center hierarchy for a few seconds. The
  // persistent gate guide returns as soon as the beat clears.
  if (storyBeat && !storyBeat.hidden) {
  ...
  const top = THREE.MathUtils.clamp(50 - point.y * 50, 18, 86);
...
function updateLandmarkGuide() {
  ...
  const candidates = authoredScenarioLandmarks
    ...
    .map((object) => {
      ...
      const actorDistance = point.distanceTo(actor);
      return { object, definition, point, actorDistance, score: priority + centerDistance * 0.18 + actorDistance * 0.4 };
    })
    .filter(({ point, actorDistance }) =>
      Math.abs(point.x) <= viewport.aspect * 0.92 &&
      Math.abs(point.y) <= 0.90 &&
      actorDistance <= 0.92
    )
  ...
  const encounter = encounterForLandmark(definition.landmark.id);
  landmarkGuideState.textContent = object.userData.finalNode
    ...
    : object.userData.finalPortal
      ...
      : definition.activated
        ? `ACTIVO · ${encounter.reward}`
        : encounter.requiresStabilizer && !scenarioGravity.status.active
          ? "ESPACIO + E · ESTABILIZÁ Y ESCANEÁ"
      : definition.state === "targetable"
        ? "ESTRUCTURA IDENTIFICADA"
        : `MANTENÉ E · NODOS ${Math.min(3, Math.floor((definition.scanProgress || 0) * 3) + 1)}/3`;
...
function updateTransition(delta, elapsed) {
  ...
  const corridorRamp = Math.min(0.82, transition.duration * 0.28);
  const entrance = THREE.MathUtils.clamp(transition.time / corridorRamp, 0, 1);
  const exit = THREE.MathUtils.clamp((transition.time - (transition.duration - corridorRamp)) / corridorRamp, 0, 1);
  ...
  interstageOverlay.material.uniforms.uOpacity.value = corridor * (0.46 + 0.16 * Math.sin(Math.PI * t));
  interstageOverlay.material.uniforms.uColor.value.set(destination.accent);
  interstageOverlay.scale.set(viewport.aspect, 1, 1);
  interstageStreaks.forEach((streak, index) => {
    const radial = 0.12 + ((t * streak.userData.speed * 2.4 + streak.userData.phase) % 1) * 1.25;
    const angle = streak.userData.angle;
    streak.position.set(Math.cos(angle) * radial * viewport.aspect, Math.sin(angle) * radial, 0.18);
    streak.scale.x = 0.44 + radial * 0.42;
    streak.material.opacity = corridor * (0.025 + 0.040 * Math.sin(Math.PI * t)) * (0.72 + (index % 3) * 0.12);
  });
...
function updateIntegratedBackground(delta, elapsed, travelVelocity) {
  ...
  for (const asteroid of integratedBackground.asteroids) {
    ...
    if (asteroid.userData.spawnRevealTime >= 0) asteroid.userData.spawnRevealTime += delta;
    const spawnReveal = asteroid.userData.spawnRevealTime >= 0
      ? THREE.MathUtils.smoothstep(asteroid.userData.spawnRevealTime, 0.08, 0.82)
      : 1;
    ...
    asteroid.scale.setScalar((0.86 + spawnReveal * 0.14) * (1 + hitPulse + hoverPulse));
    setGroupOpacity(asteroid, destroyedFade * spawnReveal);
    ...
    if (asteroid.userData.objective) {
      ...
      if (halo) {
        halo.material.opacity = (0.40 + Math.sin(elapsed * 2.8) * 0.08 + hoverPulse) * destroyedFade * spawnReveal;
...
function updateWorldCompositionAuthority(rawDelta) {
  ...
  const hasLandmarkCandidate = candidates.some((candidate) => candidate.record.kind === "landmark");
  worldCompositionTelemetry.violations = assertComposition(result).filter(
    (violation) => violation.code !== "landmark-minimum" || hasLandmarkCandidate,
  );
...
function activeNavigationObjective() {
  const actor = state.controlMode === "astronaut" ? astronautState.position : state.position;
  if (mission01.state === "small_asteroids" && missionPrimaryLandmarkPending()) {
    const entry = primaryLandmarkEntryForStage(mission01.currentStageIndex);
    if (entry?.object) {
      return {
        kind: "landmark",
        id: entry.id,
        label: entry.landmark.name,
        point: backgroundObjectScreenPoint(entry.object, new THREE.Vector2()),
        worldDistance: Math.hypot(
          entry.object.userData.base.x - state.worldOffset.x,
          entry.object.userData.base.y - state.worldOffset.y,
        ),
      };
    }
  }
  ...
  if (["small_asteroids", "large_obstacle"].includes(mission01.state) && activeTargets.length) {
    ...
    return {
      ...
      label: mission01.state === "large_obstacle" ? "NÚCLEO INESTABLE" : "FRAGMENTO DE SEÑAL",
      point: backgroundObjectScreenPoint(target, new THREE.Vector2()),
      worldDistance: target.userData.base
        ? Math.hypot(target.userData.base.x - state.worldOffset.x, target.userData.base.y - state.worldOffset.y)
        : null,
    };
  }
  if (mission01.state === "relic" && relicGroup.visible) {
    return { kind: "reliquia", id: "mission-relic", label: "RELIQUIA LIBERADA", point: new THREE.Vector2(relicGroup.position.x, relicGroup.position.y), worldDistance: 0 };
  }
  if (mission01.state === "final_nodes") {
    ...
    if (node) return { kind: "nodo", id: node.userData.landmarkId, label: "NODO GRAVITACIONAL", point: backgroundObjectScreenPoint(node, new THREE.Vector2()) };
  }
  if (mission01.state === "final_portal") {
    const portal = authoredScenarioLandmarks.find((object) => object.userData.landmarkId === "relic_portal");
    if (portal) return { kind: "portal", id: "relic_portal", label: "PORTAL DE RELIQUIA", point: backgroundObjectScreenPoint(portal, new THREE.Vector2()) };
  }
  if (["completed_region", "unlocked"].includes(mission01.state)) {
    const gate = activeForwardGate()?.object;
    if (gate) return { kind: "gate", id: gate.userData.gate?.name || "gate", label: gate.userData.gate?.name || "GATE", point: backgroundObjectScreenPoint(gate, new THREE.Vector2()) };
  }
  return null;
}

function updateObjectiveGuide(objective, from) {
  if (!objectiveGuide || !objectiveGuideArrow || !objectiveGuideLabel || !objectiveGuideDistance || !objective) {
    if (objectiveGuide) objectiveGuide.hidden = true;
    return;
  }
  const point = objective.point;
  const offscreen = Math.abs(point.x) > viewport.aspect * 0.76 || Math.abs(point.y) > 0.74;
  if (!offscreen || objective.kind === "gate") {
    objectiveGuide.hidden = true;
    return;
  }
  const edgeX = THREE.MathUtils.clamp(point.x, -viewport.aspect + 0.20, viewport.aspect - 0.20);
  const edgeY = THREE.MathUtils.clamp(point.y, -0.76, 0.72);
  objectiveGuide.style.left = `${THREE.MathUtils.clamp(((edgeX / viewport.aspect) * 0.5 + 0.5) * viewport.width, 112, viewport.width - 112)}px`;
  objectiveGuide.style.top = `${THREE.MathUtils.clamp((0.5 - edgeY * 0.5) * viewport.height, 210, viewport.height - 78)}px`;
  const direction = point.clone().sub(from).normalize();
  objectiveGuide.style.setProperty("--objective-angle", `${Math.atan2(direction.x, direction.y) * 180 / Math.PI}deg`);
  objectiveGuideLabel.textContent = objective.label || objective.kind.toUpperCase();
  objectiveGuideDistance.textContent = Number.isFinite(objective.worldDistance)
    ? `${Math.max(1, Math.ceil(objective.worldDistance))}u · SEGUÍ LA RUTA CYAN`
    : "SEGUÍ LA RUTA CYAN";
  objectiveGuide.classList.toggle("is-landmark", objective.kind === "landmark");
  objectiveGuide.classList.toggle("is-relic", objective.kind === "reliquia" || objective.kind === "portal");
  objectiveGuide.hidden = false;
}
...
function updateNavigationPath(elapsed) {
  ...
  for (let index = 0; index < NAVIGATION_PATH_POINT_COUNT; index += 1) {
    const localT = (index + 1) / (NAVIGATION_PATH_POINT_COUNT + 1);
    const t = localT * Math.min(0.46, 0.22 + distance * 0.12);
    const wave = Math.sin(Math.PI * localT) * Math.sin(elapsed * 0.9) * 0.08;
    const point = from.clone().lerp(to, t).addScaledVector(perpendicular, Math.sin(Math.PI * localT) * (0.28 + wave));
  ...
  navigationPathMaterial.opacity = 0.24 + Math.sin(elapsed * 2.4) * 0.045;
  ...
  navigationPathState.worldDistance = Number.isFinite(objective.worldDistance)
    ? Number(objective.worldDistance.toFixed(1))
    : null;
}
...
function updateTargetDiscovery(delta) {
  ...
  for (const target of targets) {
    ...
    if (previousState !== discovery.state && discovery.state === "targetable") {
      ...
      const astronautTarget = target.userData.missionRole === "small";
      setCompanionAimFeedback(
        astronautTarget
          ? "FRAGMENTO FIJADO · CLIC PARA DESPLEGAR EVA"
          : "NÚCLEO FIJADO · DISPARO PESADO DISPONIBLE",
      );
      const phaseStory = `${mission01.currentStageIndex}:${target.userData.missionRole}`;
      if (lastTargetPhaseStory !== phaseStory) {
        lastTargetPhaseStory = phaseStory;
        showStoryBeat(
          astronautTarget ? "SEÑAL LOCALIZADA" : "AMENAZA LOCALIZADA",
          astronautTarget ? "FRAGMENTO FIJADO" : "NÚCLEO INESTABLE",
          astronautTarget
            ? "Clickeá el objetivo: el astronauta se despliega, dispara y vuelve a la nave."
            : "Acercate y clickeá el núcleo para usar el cañón pesado de la nave.",
          { duration: 3200, accent: astronautTarget ? "#72eaff" : "#ff72df" },
        );
      }
    }
  }
...
function spawnFragmentTransfer(point) {
  const sprite = new THREE.Sprite(
    new THREE.SpriteMaterial({
      map: visualPackTextures.gemCore,
      color: 0x74eaff,
      transparent: true,
      opacity: 0.82,
      blending: THREE.AdditiveBlending,
...
function completeSmallMissionTargets() {
  ...
  showStoryBeat(
    `ACTO ${mission01.currentStageIndex + 1} · CONTENCIÓN`,
    "NÚCLEOS INESTABLES",
    "Piloteo restaurado. Acercate y usá el disparo pesado de la nave.",
    { duration: 3600, accent: "#ff72df" },
  );
  for (const [index, obstacle] of mission01.largeObstacles.entries()) {
    obstacle.userData.active = true;
    obstacle.userData.spawnRevealTime = 0;
...
function revealMissionRelic(sourceTarget) {
  ...
  showStoryBeat(
    `ACTO ${mission01.currentStageIndex + 1} · RESONANCIA`,
    "RELIQUIA LIBERADA",
...
function acquireStageGem() {
  ...
  if (mission01.gems < nextGemCount) {
    ...
    showStoryBeat(
      `GEMA ${mission01.gems}/3 ACOPLADA`,
      currentTurboUnlock().label,
      mission01.gems >= 3
        ? "Las tres resonancias están sincronizadas. La ruta final puede abrirse."
        : "El sistema de propulsión mejoró. Mantené F durante el viaje al próximo gate.",
      { duration: 4200, accent: "#ffb56f" },
    );
  }
  updateGemHud();
  saveProgress();
...
function activateFinalNode(node) {
  ...
  setCompanionAimFeedback(`NODO SINCRONIZADO · ${mission01.finalNodesActivated}/3`);
  updateMissionHud(
    "RELIC CORE",
...
function touchMissionRelic() {
  ...
  mission01.state = "collecting_relic";
  mission01.relicState = "collecting";
  triggerCameraCue("pickup");
  stopMissionAudio("relic_idle");
  playMissionAudio("relic_touch");
  playMissionAudio("energy_transfer");
  triggerAstronautAction();
  gemPickupState.active = true;
  gemPickupState.time = 0;
  gemPickupState.start.set(relicGroup.position.x, relicGroup.position.y);
  gemPickupState.hand.copy(astronautState.position).add(new THREE.Vector2(0.035, 0.025));
  gemPickupState.ship.set(shipGroup.position.x, shipGroup.position.y);
  updateMissionHud("RESONANCIA CAPTURADA", "ACOPLANDO GEMA A LA NAVE", currentMissionConfig().subtitle);
  showStoryBeat(
    `ACTO ${mission01.currentStageIndex + 1} · RECUPERACIÓN`,
    "GEMA MAGNETIZADA",
    "La resonancia pasa de la mano del astronauta al núcleo de la nave.",
    { duration: 3200, accent: "#ff86e7" },
  );
  robotCompanion.focus = "discovery";
  robotCompanion.focusTimer = 3.2;
  robotCompanion.message = "GEMA MAGNETIZADA · TRANSFERENCIA EN CURSO";
  robotCompanion.pulse = 1;
  updateRobotPanel();
}

function completeMissionRelicCollection() {
  if (!gemPickupState.active) return;
  gemPickupState.active = false;
  ...
  window.setTimeout(() => {
    ...
    showStoryBeat(
      `SECTOR ${mission01.currentStageIndex + 1} ESTABILIZADO`,
      `GATE → ${nextScenario.name}`,
      "Piloteá siguiendo la ruta magenta. El próximo stage inicia al cruzar el corredor.",
      { duration: 4200, accent: nextScenario.accent },
    );
    syncRobotCompanion("completed_region");
    updateStageHud();
    saveProgress();
...
function launchShotAtPoint(point, shooter = "ship", options = {}) {
  ...
  if (shooter === "astronaut" && astronautSprite) enterAstronautMode(options.relic ? "relic" : "combat");
  if (shooter === "ship") enterShipMode();
...
function updateInteractionFx(delta) {
  ...
  for (let i = activeImpacts.length - 1; i >= 0; i -= 1) {
    ...
    if (impact.userData.transfer) {
      const progress = THREE.MathUtils.clamp(t, 0, 1);
      const eased = THREE.MathUtils.smoothstep(progress, 0, 1);
      const target = new THREE.Vector2(shipGroup.position.x, shipGroup.position.y);
      const arc = Math.sin(progress * Math.PI) * 0.15;
      impact.position.set(
        THREE.MathUtils.lerp(impact.userData.start.x, target.x, eased),
        THREE.MathUtils.lerp(impact.userData.start.y, target.y, eased) + arc,
        0.15,
      );
      const pulse = 0.82 + Math.sin(progress * Math.PI * 4) * 0.12;
      impact.scale.setScalar(impact.userData.baseScale * pulse * (1 - progress * 0.36));
      impact.material.opacity = impact.userData.baseOpacity * (1 - THREE.MathUtils.smoothstep(progress, 0.72, 1));
      if (progress >= 1) {
...
function updateMission01(delta, elapsed) {
  if (mission01.relicGroup?.visible) {
    ...
    if (gemPickupState.active) {
      gemPickupState.time += delta;
      const pickupT = THREE.MathUtils.clamp(gemPickupState.time / gemPickupState.duration, 0, 1);
      gemPickupState.hand.copy(astronautState.position).add(new THREE.Vector2(0.035, 0.025));
      gemPickupState.ship.set(shipGroup.position.x, shipGroup.position.y);
      if (pickupT < 0.38) {
        const handT = THREE.MathUtils.smoothstep(pickupT / 0.38, 0, 1);
        relicGroup.position.set(
          THREE.MathUtils.lerp(gemPickupState.start.x, gemPickupState.hand.x, handT),
          THREE.MathUtils.lerp(gemPickupState.start.y, gemPickupState.hand.y, handT),
          0.14,
        );
      } else {
        const shipT = THREE.MathUtils.smoothstep((pickupT - 0.38) / 0.62, 0, 1);
        const arc = Math.sin(shipT * Math.PI) * 0.10;
        relicGroup.position.set(
          THREE.MathUtils.lerp(gemPickupState.hand.x, gemPickupState.ship.x, shipT),
          THREE.MathUtils.lerp(gemPickupState.hand.y, gemPickupState.ship.y, shipT) + arc,
          0.14,
        );
      }
      relicGroup.scale.setScalar(1 - THREE.MathUtils.smoothstep(pickupT, 0.36, 1) * 0.76);
      relicGroup.rotation.z += delta * (0.8 + pickupT * 3.2);
      if (pickupT >= 1) completeMissionRelicCollection();
    }
...
function enterAstronautMode(reason = "manual") {
  ...
  if (reason === "manual") {
    robotCompanion.focus = "turbo";
    robotCompanion.focusTimer = 4.8;
    robotCompanion.message = "MICROPROPULSOR EVA · MANTENÉ F CON WASD";
    robotCompanion.pulse = 1;
  } else if (reason === "relic") {
    robotCompanion.focus = "discovery";
    robotCompanion.focusTimer = 3.8;
    robotCompanion.message = "EVA ACTIVO · ACERCATE Y TOCÁ LA RELIQUIA";
    robotCompanion.pulse = 1;
  }
  updateRobotPanel();
  updateStageHud();
...
function updateAstronaut(delta, elapsed, controlVelocity) {
  ...
  const commandedVelocity = isControlled ? controlVelocity : visualVelocity;
  const astronautMoving = isControlled && commandedVelocity.length() > 0.10 && !state.transition;
  const astronautDirection = astronautMoving
    ? directionFromAngle(Math.atan2(commandedVelocity.y, commandedVelocity.x))
    : "idle";
  astronautThrusters.setState(
    astronautDirection,
    astronautMoving ? speedState.turboPulse > 0.08 ? "turbo" : "normal" : "idle",
    astronautSprite.scale.x,
    astronautSprite.scale.y,
    0,
  );
  evaMicroJetGroup.visible = astronautMoving && speedState.turboPulse > 0.08;
  if (evaMicroJetGroup.visible) {
    evaMicroJetGroup.rotation.z = Math.atan2(commandedVelocity.y, commandedVelocity.x);
    const jetPulse = 0.28 + Math.sin(elapsed * 18) * 0.05 + speedState.turboPulse * 0.10;
    for (const jet of evaMicroJetGroup.children) jet.material.uniforms.uOpacity.value = jetPulse;
  }
}
  ...
  if (event.key === " " || event.key === "Enter") {
    ...
    if (event.key === " ") {
      if (scenarioGravity.activateStabilizer()) {
        ...
        robotCompanion.focus = "stabilizer";
        robotCompanion.focusTimer = 4.2;
        robotCompanion.message = "ESTABILIZADOR ACTIVO · GRAVEDAD AL 24%";
        robotCompanion.pulse = 1;
        updateRobotPanel();
        playAudioEvent("aim_stabilize");
        showStoryBeat(
          ...
          "Gravedad reducida al 24% durante 2.5s. Después entra en recarga.",
          { duration: 2600, accent: "#72eaff" },
        );
      } else {
        robotCompanion.focus = "stabilizer";
        robotCompanion.focusTimer = 2.4;
        robotCompanion.message = `ESTABILIZADOR · RECARGA ${Math.ceil(scenarioGravity.status.cooldown)}s`;
        robotCompanion.pulse = 1;
        updateRobotPanel();
      }
      return;
    ...
    else setCompanionAimFeedback("SECTOR BLOQUEADO · RECUPERÁ LA GEMA");
  }
});
...
window.addEventListener("pointerdown", (event) => {
  ...
  if (mission01.relicState === "collectible" && point.distanceTo(relicPoint) < 0.18) {
    setCompanionAimFeedback("ACERCATE CON EL ASTRONAUTA · LA GEMA SE ACOPLA POR PROXIMIDAD");
    playAudioEvent("route_detected_ping");
    return;
  }
...
function animate() {
  ...
  stabilizerFieldFx.update(
    rawDelta,
    elapsed,
    scenarioGravity.status,
    scenarioGravitySample,
    state.controlMode === "astronaut" ? astronautState.position : state.position,
  );
  updateAbilityHud();
...
if (qaRoute === "stage1Edge") {
  window.setTimeout(() => {
    const scenario = scenarioForStage(0);
    mission01.gems = 0;
    setWorldStage(0);
    applyStage(0);
    setWorldOffset(scenario.center.x, scenario.center.y);
    startMissionForStage(0);
    const primary = primaryLandmarkEntryForStage(0);
    if (primary) {
      primary.activated = false;
      primary.state = "unknown";
      primary.scanProgress = 0;
      primary.signalStrength = 0;
    }
    setWorldOffset(scenario.bounds.minX, scenario.bounds.minY);
    enterShipMode();
    hideMenu();
  }, 350);
}
...
if (qaRoute === "astronautTurbo") {
  window.setTimeout(() => {
    ...
    hideMenu();
    enterAstronautMode();
    astronautState.position.set(0.16, 0.02);
    input.keys.add("w");
    input.keys.add("f");
    updateMissionHud("MICROPROPULSOR EVA", "MANTENÉ F CON WASD · TETHER LIMITADO", "QA / ASTRONAUTA");
  }, 350);
}
if (params.get("autoStabilizer") === "1") {
  window.setTimeout(() => {
    scenarioGravity.activateStabilizer();
    robotCompanion.focus = "stabilizer";
    robotCompanion.focusTimer = 4;
  }, 720);
}
if (qaRoute === "landmark") {
  window.setTimeout(() => {
    ...
    setWorldOffset(beacon.x - 2.45, beacon.y);
    enterAstronautMode();
    astronautState.position.set(0.155, 0.05);
...
if (qaRoute === "gemPickup") {
  window.setTimeout(() => {
    ...
    mission01.state = "relic";
    mission01.relicState = "collectible";
    relicGroup.position.set(0.28, 0.18, 0.12);
    relicGroup.visible = true;
    enterAstronautMode();
    astronautState.position.set(0.20, 0.12);
    hideMenu();
    gemPickupState.duration = 3.2;
    window.setTimeout(touchMissionRelic, 420);
  }, 350);
}
