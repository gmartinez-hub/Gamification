...
class CompleteV4Controller {
  ...
  createGravityIndicator() {
    const root = document.createElement("div"); root.className = "v4-gravity-indicator"; root.innerHTML = `<img src="/assets/runtime/v4/gravity/gravity_vector_arc.png" alt="Vector gravitacional"><span>GRAVEDAD</span>`; document.body.appendChild(root); return root;
  }

  createRouteIndicator() {
    const root = document.createElement("div"); root.className = "v4-route-indicator"; root.innerHTML = `<span class="v4-route-arrow">▲</span><div><strong>RUMBO</strong><small></small></div>`; document.body.appendChild(root); return root;
  }
  ...
  applyForces({ rawDelta, elapsed, inputVelocity }) {
    if (this.map.open) { inputVelocity.multiplyScalar(0.80); return this.gravity.lastSample; }
  ...
  updateRouteIndicator() {
    ...
    const name = REGIONS[targetIndex].name;
    this.routeIndicator.querySelector("small").textContent = `${name} · ${Math.round(d)} u`;
  ...
  update({ rawDelta, delta, elapsed, shipVelocity, travelVelocity }) {
    ...
    this.floatingOrigin.update(this.context.state.worldOffset);
    this.world.update({ floatingOrigin: this.floatingOrigin, elapsed });
